...
import { z } from 'zod';
import { JSON as TestJSON, Types } from '../../src/index.js';

export function createTests(SchemaJSON: typeof TestJSON) {
  const { parse, stringify } = SchemaJSON;
  ...
  function testAdminObject(
    ...
    const match: AdminType = {
      ...(dirty ? adminInputDirty : adminInputStrict),
    };
